...
# ── External AMTL App Registry ────────────────────────────────────
EXTERNAL_APPS = {
    "writer": {"name": "CK Writer", "port": 5004, "health": "/", "delegate": "/api/task"},
    "learning": {"name": "Learning Assistant", "port": 5002, "health": "/api/health", "delegate": "/api/task"},
    "peterman": {"name": "Peterman", "port": 5008, "health": "/api/health", "delegate": "/api/task"},
    "costanza": {"name": "Costanza", "port": 5001, "health": "/api/health", "delegate": "/api/task"},
    "genie": {"name": "Genie", "port": 8000, "health": "/api/health", "delegate": "/api/task"},
    "junk-drawer": {"name": "Junk Drawer", "port": 5005, "health": "/", "delegate": "/api/task"},
}
...
class Orchestrator:
    ...
    # Route tasks to CK Writer, Learning Assistant, Peterman,
    # Costanza, Genie, Junk Drawer — track completion and report.

    def __init_delegation(self):
        """Initialise delegation tracking (called lazily)."""
        if not hasattr(self, "_delegated_tasks"):
            self._delegated_tasks: list[dict] = []

    def _ping_app(self, app_id: str) -> bool:
        """Check if an external app is reachable."""
        app = EXTERNAL_APPS.get(app_id)
        if not app:
            return False
        try:
    ...
    def delegate_task(self, app_id: str, task_type: str,
                      payload: dict, priority: str = "normal") -> dict:
        """Delegate a task to an external AMTL app.

        Args:
            app_id: Key from EXTERNAL_APPS (e.g. 'writer', 'genie')
            task_type: What to do (e.g. 'write_draft', 'analyse_receipt')
            payload: Task-specific data
            priority: 'low', 'normal', 'high', 'urgent'

        Returns dict with task_id, status, and app response (if reachable).
        """
        self.__init_delegation()
        app = EXTERNAL_APPS.get(app_id)
        if not app:
            return {"error": f"Unknown app: {app_id}", "known_apps": list(EXTERNAL_APPS.keys())}

        task_id = f"del-{app_id}-{datetime.now().strftime('%Y%m%d%H%M%S')}-{len(self._delegated_tasks)}"
        task = {
            "task_id": task_id,
            "app_id": app_id,
            "app_name": app["name"],
        ...
        # Try to send to the app
        try:
            url = f"http://localhost:{app['port']}{app['delegate']}"
            body = json.dumps({
                "task_id": task_id,
                "task_type": task_type,
                "priority": priority,
            ...
            with urllib.request.urlopen(req, timeout=2) as resp:
                ...
                logger.info("Delegated %s to %s (:%.0f): %s",
                            task_type, app["name"], app["port"], task_id)
        except urllib.error.URLError as e:
            task["status"] = "app_offline"
            task["error"] = f"{app['name']} (:{app['port']}) is not reachable: {e.reason}"
            logger.warning("Delegation to %s failed — app offline: %s", app["name"], e.reason)
        except Exception as e:
            task["status"] = "error"
            task["error"] = str(e)
            logger.warning("Delegation to %s failed: %s", app["name"], e)

        self._delegated_tasks.append(task)
        self._log("orchestrator", app_id, f"delegate:{task_type}",
                   f"task_id={task_id}, status={task['status']}")
        return task

    def delegate_writing(self, content_type: str, topic: str,
                         notes: str = "", tone: str = "professional") -> dict:
        """Delegate writing to CK Writer (:5004)."""
        return self.delegate_task("writer", "write_draft", {
    ...
    def delegate_learning(self, skill: str, context: str = "") -> dict:
        """Delegate a learning session to Learning Assistant (:5002)."""
        return self.delegate_task("learning", "learning_session", {
            "skill": skill,
            "context": context,
        })

    def delegate_brand_check(self, url: str = "", topic: str = "") -> dict:
        """Delegate brand intelligence to Peterman (:5008)."""
        return self.delegate_task("peterman", "brand_audit", {
            "url": url,
            "topic": topic,
        })

    def delegate_mental_model(self, situation: str, model: str = "auto") -> dict:
        """Delegate to Costanza for mental model analysis (:5001)."""
        return self.delegate_task("costanza", "apply_model", {
            "situation": situation,
            "model": model,
        })

    def delegate_financial(self, query_type: str = "summary",
                           period: str = "this_month") -> dict:
        """Delegate to Genie for financial queries (:8000)."""
        return self.delegate_task("genie", query_type, {
            "period": period,
        })

    def get_delegated_tasks(self, limit: int = 20,
                             app_id: str = None) -> list[dict]:
        """Return recent delegated tasks, optionally filtered by app."""
        self.__init_delegation()
        tasks = self._delegated_tasks
        if app_id:
            tasks = [t for t in tasks if t["app_id"] == app_id]
        return tasks[-limit:]

    def get_app_status(self) -> dict:
        """Check which external apps are reachable."""
        results = {}
        for app_id, app in EXTERNAL_APPS.items():
            reachable = self._ping_app(app_id)
            results[app_id] = {
                "name": app["name"],
                "port": app["port"],
                "reachable": reachable,
